...
Additionally, ~git~ installedfrom another package manager like ~brew~
or ~nix~ seems to be slower than the native executable. Profile the
~git~ executable you're running against the one at ~/usr/bin/git~, and
if you notice a notable difference try setting ~magit-git-executable~
to it.
